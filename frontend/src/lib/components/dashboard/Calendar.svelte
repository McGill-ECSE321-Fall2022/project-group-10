<div class="calendar">
	...
	<span class="day-name" on:click={()=>dispatch('headerClick',header)} on:keydown = {null} >{header}</span>
	{/each}

	{#each days as day}
		{#if day.enabled}
			<span class="day" on:click={()=>dispatch('dayClick',day)} on:keydown = {null}>{day.name}</span>
		{:else}
			<span class="day day-disabled" on:click={()=>dispatch('dayClick',day)} on:keydown = {null}>{day.name}</span>
		{/if}
	{/each}
...
<style>
  ...
  .task--primary {
    ...
    border: 0;
    border-radius: 14px;
    box-shadow: 0 0.5rem 1rem rgba(71, 134, 255, 0.4);
  }
  .task-detail {
    position: absolute;
    left: 0;
